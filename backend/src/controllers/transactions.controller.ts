...
export async function listTransactions(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const { page = '1', limit = '20', status, bankName, utr, startDate, endDate } = req.query as Record<string, string>;
    ...
    const [data, total] = await Promise.all([
      prisma.bankTransaction.findMany({
        ...
        select: {
          id: true, utr: true, amount: true, currency: true,
          transactionType: true, bankName: true, status: true,
          parseMethod: true, parseConfidence: true,
          receivedAt: true, verifiedAt: true, sheetsSynced: true,
          payerName: true, employeeId: true,
        },
      }),
...
export async function exportTransactions(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    const transactions = await prisma.bankTransaction.findMany({ orderBy: { receivedAt: 'desc' }, take: 5000 });

    if (format === 'csv') {
      const headers = ['UTR', 'Payer Name', 'Amount', 'Currency', 'Type', 'Bank', 'Status', 'Employee ID', 'Received At'];
      const rows = transactions.map((t) =>
        [
          t.utr,
          t.payerName ? `"${t.payerName.replace(/"/g, '""')}"` : '',
          t.amount,
          t.currency,
          t.transactionType,
          t.bankName,
          t.status,
          t.employeeId ? `"${t.employeeId.replace(/"/g, '""')}"` : '',
